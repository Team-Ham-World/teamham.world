...
import { GET, POST } from "@/app/api/puff/suipuff/leaderboard/route";
...
import * as leaderboardModule from "@/lib/puff/suipuff-leaderboard";
...
vi.mock("@/lib/puff/suipuff-leaderboard", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/puff/suipuff-leaderboard")>();
  return {
    ...actual,
    getSuipuffLeaderboard: vi.fn(),
    saveSuipuffHighScore: vi.fn(),
  };
});

describe("Suipuff leaderboard endpoint", () => {
  ...
  function request(
    ...
    return new Request("https://teamham.world/api/puff/suipuff/leaderboard", {
      method,
      headers,
      body:
        options.rawBody ??
  ...
  it.each(["disabled", "invalid"])(
    ...
    async (mode) => {
      ...
      expect(leaderboardModule.getSuipuffLeaderboard).not.toHaveBeenCalled();
  ...
  it("returns the authenticated Suipuff snapshot without exposing an identifier", async () => {
    setTestEnv(VALID_PROD_ENV);
    const token = generateSessionToken();
    const tokenHash = mockMember(token);
    // 7 is deliberately not a multiple of five — Suipuff scores are unquantised.
    vi.mocked(leaderboardModule.getSuipuffLeaderboard).mockResolvedValueOnce({
      personalBest: 7,
      scores: [{ rank: 1, username: "hamfriend", score: 7, mine: true }],
    });
    ...
    expect(leaderboardModule.getSuipuffLeaderboard).toHaveBeenCalledWith(
      accountId,
      expect.any(String),
    );
    expectProtected(response);
  ...
  it("degrades an authenticated GET eligibility race to the signed-out payload", async () => {
    ...
    const token = generateSessionToken();
    mockMember(token);
    vi.mocked(leaderboardModule.getSuipuffLeaderboard).mockResolvedValueOnce(null);
  ...
  it("requires authentication before accepting a score", async () => {
    ...
    const response = await POST(
      request("POST", { body: { score: 7 }, origin: "https://teamham.world" }),
    );

    expect(response.status).toBe(401);
    expect(await response.json()).toEqual({ error: "authentication_required" });
    expect(leaderboardModule.saveSuipuffHighScore).not.toHaveBeenCalled();
    expectProtected(response);
  });
  ...
  it.each([undefined, "https://evil.example"])(
    ...
    async (origin) => {
      ...
      const response = await POST(request("POST", { token, body: { score: 7 }, origin }));
  ...
  it.each([
    [{ score: 7, extra: true }, "extra key"],
    [{ score: -7 }, "below range"],
    [{ score: 1_000_007 }, "above range"],
    [{ score: 2.5 }, "fraction"],
    [{ score: "7" }, "wrong type"],
    [{ score: null }, "null"],
  ...
  it("accepts scores that are not multiples of five", async () => {
    setTestEnv(VALID_PROD_ENV);
    const token = generateSessionToken();
    mockMember(token);
    vi.mocked(leaderboardModule.saveSuipuffHighScore).mockResolvedValueOnce(7);
    vi.mocked(leaderboardModule.getSuipuffLeaderboard).mockResolvedValueOnce({
      personalBest: 7,
      scores: [{ rank: 1, username: "hamfriend", score: 7, mine: true }],
    });
    ...
    const response = await POST(
      request("POST", { token, body: { score: 7 }, origin: "https://teamham.world" }),
    );

    expect(response.status).toBe(200);
    expect(leaderboardModule.saveSuipuffHighScore).toHaveBeenCalledWith(
      accountId,
      7,
      expect.any(String),
    );
  ...
  it("returns 401 when eligibility changes during a save", async () => {
    ...
    const token = generateSessionToken();
    mockMember(token);
    vi.mocked(leaderboardModule.saveSuipuffHighScore).mockResolvedValueOnce(null);
  ...
  it("saves a score and returns the refreshed snapshot", async () => {
    ...
    const token = generateSessionToken();
    mockMember(token);
    vi.mocked(leaderboardModule.saveSuipuffHighScore).mockResolvedValueOnce(24);
    vi.mocked(leaderboardModule.getSuipuffLeaderboard).mockResolvedValueOnce({
      personalBest: 24,
      scores: [{ rank: 1, username: "hamfriend", score: 24, mine: true }],
    });

    const response = await POST(
      request("POST", { token, body: { score: 24 }, origin: "https://teamham.world" }),
    );
  ...
  it.each(["session", "get", "save"])(
    ...
    async (failure) => {
      ...
      if (failure === "session") {
        ...
        mockMember(token);
        vi.mocked(leaderboardModule.getSuipuffLeaderboard).mockRejectedValueOnce(
          new Error("database detail"),
        );
        response = await GET(request("GET", { token }));
      } else {
        mockMember(token);
        vi.mocked(leaderboardModule.saveSuipuffHighScore).mockRejectedValueOnce(
          new Error("database detail"),
        );
        response = await POST(
          request("POST", { token, body: { score: 7 }, origin: "https://teamham.world" }),
