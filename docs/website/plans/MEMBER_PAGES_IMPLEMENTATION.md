...
Add `migrations/0005_member_pages.sql` as the initial committed member-page
migration, followed by the additive `migrations/0006_member_social_links.sql`
when social profiles are introduced.
...
| `social_links` | Non-null JSON object, empty by default, containing only server-supported platform keys and absolute HTTPS profile URLs. Added additively by `migrations/0006_member_social_links.sql`. |
...
7. Render saved social profiles as monochrome SVG-logo stickers beside the personal-site action. Keep platform order stable, use accessible names, and use the existing HAM ink, paper, blue, borders, and offset-shadow tokens.
...
- field limits, HTTPS URL checks, reserved slugs, malformed social-link objects, malformed showcases, Open Graph parsing, and private-address rejection;
