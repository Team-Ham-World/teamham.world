...
export async function GET(request: Request): Promise<Response> {
  ...
  if (member.kind === "signed-out") {
    return json({ authenticated: false, username: null, personalBest: 0, scores: [] });
  }

  try {
    const snapshot = await getPuffLeaderboard(member.account.id, member.databaseUrl);
    return json({
      authenticated: true,
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (
    ...
    !("score" in body) ||
    !isValidPuffScore(body.score)
  ) {
    return json({ error: "invalid_score" }, 400);
  }

  try {
    await savePuffHighScore(member.account.id, body.score, member.databaseUrl);
    const snapshot = await getPuffLeaderboard(member.account.id, member.databaseUrl);
    return json({
      authenticated: true,
      username: member.account.username,
