...
import {
  getSuipuffLeaderboard,
  isValidSuipuffScore,
  saveSuipuffHighScore,
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const snapshot = await getSuipuffLeaderboard(member.account.id, member.databaseUrl);
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (
    ...
    !("score" in body) ||
    !isValidSuipuffScore(body.score)
  ) {
    return json({ error: "invalid_score" }, 400);
  }

  try {
    const saved = await saveSuipuffHighScore(member.account.id, body.score, member.databaseUrl);
